...
const getAllEvents = async (req, res) => {
  let filter = pick(req.query, ["search", "status", "eventDate"]);
  ...
  if (filter.hasOwnProperty("search")) {
    filter = {
      ...filter,
      name: {
        $regex: new RegExp(`.*${filter.search.toLowerCase()}.*`),
        $options: "i",
      },
      category: {
        $regex: new RegExp(`.*${filter.search.toLowerCase()}.*`),
  ...
  if (req.query.hasOwnProperty("eventDate")) {
    filter = {
      ...filter,
...
const getEventById = async (req, res) => {
  const result = await Events.findById({
    status: "ACTIVE",
    _id: req.params.id,
  });
  res.send({ result, status: true });
